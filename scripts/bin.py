...
# Plus besoin de BASE_PATH ou de get_path compliqué
blue_bin_image   = load_image('poubelles/BlueBin.png',   scale=0.5)
yellow_bin_image = load_image('poubelles/YellowBin.png', scale=0.5)
brown_bin_image  = load_image('poubelles/BrownBin.png',  scale=0.5)
green_bin_image  = load_image('poubelles/GreenBin.png',  scale=0.5)
MAX_WASTE = 5
GAUGE_WIDTH = 50
GAUGE_HEIGHT = 8
GAUGE_OFFSET_Y = 6
...
class Collectible:
    ...
    def __init__(self, platform, game, image):
        ...
        # On centre la poubelle sur la plateforme
    ...
    def render(self, surf, scroll):
        # On applique le scroll (souvent scroll est une liste [x, y])
        ...
        # Dessin du fond de la jauge
        pygame.draw.rect(surf, GAUGE_BG_COLOR, (gauge_x, gauge_y, GAUGE_WIDTH, GAUGE_HEIGHT))

        # Dessin du remplissage
        fill_w = int(GAUGE_WIDTH * (self.waste_count / MAX_WASTE))
        if fill_w > 0:
            pygame.draw.rect(surf, self.gauge_color, (gauge_x, gauge_y, fill_w, GAUGE_HEIGHT))

        # Bordure de la jauge
        pygame.draw.rect(surf, GAUGE_BORDER, (gauge_x, gauge_y, GAUGE_WIDTH, GAUGE_HEIGHT), 1)


# --- Sous-classes simplifiées ---
